      ...
      messages: [
        {
          ...
          content: {
            type: "text",
            text: `Verify this citation using the verifyCitation tool from the scholar-sidekick MCP server. The cited title is "${title}" and the cited identifier is ${identifier}. Detect the identifier type (DOI starts with "10." — a shortDOI like "10/aabbe" is also a DOI, pass it in the doi field and the server expands it; PMID is numeric, PMCID starts with "PMC", arXiv ID matches YYYY.NNNNN or has the "arXiv:" prefix, ISBN is 10 or 13 digits, etc.) and pass it in the appropriate field. Report the verdict (matched / mismatch / ambiguous / not_found), the confidence, and — if there is any mismatch — what the resolved record at that identifier actually says, so the user can see where the cited title and the resolved title diverged. This is the check that catches the dominant AI-citation-fabrication pattern documented by Topaz et al. (Lancet 2026).`,
          },
        },
      ],
    }),
